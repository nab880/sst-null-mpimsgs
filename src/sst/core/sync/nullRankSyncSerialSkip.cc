...
NullRankSyncSerialSkip::sendData(int to_rank)
{
    ...
    SimTime_t         next_send = current_cycle + comm_map[to_rank].delay; // 0.1 - 1
...
void
NullMessageEvent::execute(void)
{
    auto exit = Simulation_impl::getSimulation()->getSimulation()->getExit();
    exit->check();
    skip->sendData(rank);
}
} // namespace SST